...
<div className="lg:hidden relative">
  ...
  <button
    onClick={() => scroll("left")}
    className="absolute left-2 top-1/2 -translate-y-1/2 z-20 bg-white/95 backdrop-blur-sm rounded-full p-2.5 shadow-xl hover:bg-white hover:scale-110 transition-all duration-200"
    aria-label="Scroll kiri"
  >
    <ChevronLeft size={20} className="text-[#18b3ab]" />
  </button>

  {/* Tombol Kanan */}
  <button
    onClick={() => scroll("right")}
    className="absolute right-2 top-1/2 -translate-y-1/2 z-20 bg-white/95 backdrop-blur-sm rounded-full p-2.5 shadow-xl hover:bg-white hover:scale-110 transition-all duration-200"
    aria-label="Scroll kanan"
  >
    <ChevronRight size={20} className="text-[#18b3ab]" />
  </button>
  ...
  >
    {categories.map((category, index) => (
      <div
        key={index}
        className="flex-none w-[220px] snap-center group cursor-pointer flex flex-col items-center"
      >
        {/* Image Container */}
        <div className="relative w-full aspect-square mx-auto transition-all duration-500 ease-out group-hover:-translate-y-3 group-hover:scale-105">
          ...
          {/* Text Label */}
          <div className="absolute bottom-[-18px] left-1/2 -translate-x-1/2 bg-[#d5dd23] rounded-[16px] h-[64px] w-[170px] flex flex-col items-center justify-center transition-all duration-500 ease-out group-hover:bg-[#c5cd13] group-hover:shadow-[0_10px_30px_rgba(213,221,35,0.4)] group-hover:-translate-y-2 group-hover:scale-105 z-10 px-1.5">
            <p className="font-semibold text-[16px] leading-[20px] text-[#383838] text-center transition-all duration-300 group-hover:scale-110">
              {category.name}
            </p>
            <p className="font-thin text-[9px] leading-[16px] text-[#302e2e] text-center transition-all duration-300 group-hover:text-[#1a1a1a]">
              {category.age}
            </p>
          </div>
        </div>
